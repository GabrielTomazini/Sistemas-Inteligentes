...
class Game:
    ...
    def update(self):
        ...
        if (
            x < 0
            or y < 0
            or x >= GameConstants.gridCellHeight
            or y >= GameConstants.gridCellWidth
        ):
            return
        ...
        for column in range(GameConstants.gridWidth):
            ...
            rects += [
                pygame.draw.rect(
                    screen,
                    color,
                    [(2 * m + w) * column + m, (2 * m + h) * row + m, w, h],
                )
            ]
...
def initialize():
    ...
    screen = pygame.display.set_mode(
        (GameConstants.screenWidth, GameConstants.screenHeight), pygame.DOUBLEBUF
    )
